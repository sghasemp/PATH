...
com1 <- com1 %>%
  mutate(Numeric = ifelse(Var1 == "Advanced degree", 5,
...
com1 <- com1 %>%
  mutate(Numeric = ifelse(Var1 == "Married", 1,
...
com1 <- com1 %>%
  mutate(Numeric = ifelse(Var1 == "Father", 1,
                            ifelse(Var1== "Mother", 2,
                                   ifelse(Var1 == "Other", 3 , 
Numeric))))
...
```{r}
# Define the mapping from parents relationship levels to numeric values
...
com2 <- com2 %>%
  mutate(NumericRelation = relation_map[parent])
...
#Calculate statistics for income, education ,and marital status, child-parent relation, child_sex
...
#calculate statistical for martial status
relationship_stats <- calculate_stats(com2$NumericRelationship)
#calculate statistical for Child_parent  status
relation_stats <- calculate_stats(com2$NumericRelation)
#calculate statistical for Child_parent  status
sex_stats <- calculate_stats(com2$NumericSex)


#calculate statistical for household
...
#calculate statistical for child number

control_childnum <- com2 %>%
  summarise(
    Mean = mean(as.numeric(num_children), na.rm = TRUE),
    Median = median(num_children, na.rm = TRUE),
    Min = min(num_children, na.rm = TRUE),
...
Surveys_CEP_stats$control_childnum<-control_childnum
Surveys_CEP_stats$relationship_stats<-relationship_stats
Surveys_CEP_stats$relation_stats<-relation_stats
Surveys_CEP_stats$sex_stats<-sex_stats
...
# Add identifiers
identifiers <- c("PFSQ5","PFSQ17", "PFSQ18","PFSQ20", "PFSQ24", "PFSQ26", "PFSQ25",  "PFSQ21","PFSQ22", "PFSQ15","PFSQ13","PFSQ2","CEBQ32","CEBQ24","CEBQ16","CEBQ10","CEBQ7","control_Age","control_stats","emotional_stats","picky_stats","income_stats", "education_stats" ,"control_household","relationship_stats" , "control_childnum","relation_stats" , "sex_stats")
