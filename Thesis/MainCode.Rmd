...
```{r}
 ...
 lm <- lm(BMIp~ Picky_Score_log + Control_Score_log + 
...
# Categorize Picky Eating Score into three categories: high, medium, low
Data <- Data %>%
  mutate(Picky_Category = case_when(
    Picky_Score_log > quantile(Picky_Score_log, 0.66) ~ "High",
    Picky_Score_log > quantile(Picky_Score_log, 0.33) ~ "Medium",
    TRUE ~ "Low"
  ))
...
```{r}
 ...
 lm2 <- lm(BMIp~ Picky_Score_log + Emotional_Score_log + 
