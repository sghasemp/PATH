...
outcome_model <- glm(Picky_Category ~ Control_Score+ SNAP +
...
ggplot(ECfinal, aes(x = Control_Score, y = Picky_Score, color = factor(SNAP))) +
  ...
  labs(title = "Control Score vs. Picky Eating by SNAP",
       x = "Control Score ", y = "Picky Eating Score",
       color = "SNAP Status") +
  theme_minimal() +
...
numeric_predictors <- c("Control_Score", "Sex", "Relationship", "Income", "Education", "Relation", "household", "num_children")
...
outcome_model2 <- glm(Picky_Category ~ Emotional_Score + SNAP + 
              Sex + Income + Education + Relation + household+ Relationship +num_children ,family = binomial, data = ECfinal)
...
# Scatter plot with separate regression lines for SNAP 0 and SNAP 1
ggplot(ECfinal, aes(x = Emotional_Score, y = Picky_Score, color = factor(SNAP))) +
  geom_point() +  
  geom_smooth(method = "lm", se = FALSE, aes(group = factor(SNAP))) +  # Add separate regression lines for each SNAP group
  labs(title = "Emotional Score vs. Picky Eating by SNAP",
       x = "Emotional Score ", y = "Picky Eating Score ",
       color = "SNAP Status") +
  theme_minimal() +
  scale_color_manual(values = c("blue", "red"),
                     labels = c("0" = "No SNAP", "1" = "SNAP"))  # Specify colors for SNAP 0 and SNAP 1
...
numeric_predictors2 <- c("Emotional_Score", "Sex", "Relationship", "Income","num_children", "Education", "Relation", "household")
...
```{r}
 ...
 lm <- lm(BMIp~ Picky_Category + Control_Score + 
                Picky_Category* Control_Score + SNAP + 
...
ggplot(Data, aes(x = Control_Score, y = BMIp, color = Picky_Category)) +
  geom_point() + 
  ...
  annotate("text", x = max(Data$Control_Score), y = min(Data$BMIp), 
...
# Calculate the correlation matrix of predictors
 correlation_matrix <- cor(Data[c("Picky_Score", "Control_Score", "SNAP", "Sex", "Income", "Education", "Relation", "household", "Relationship", "num_children")])
...
```{r}
 ...
 lm2 <- lm(BMIp~ Picky_Category + Emotional_Score + 
                Picky_Category * Emotional_Score + SNAP + 
              Sex + Income + Education + Relation + household+ Relationship +num_children, data = Data)
summary(lm2)
...
# Plot BMI vs. Emotional Score with categorized Picky Eating Score and separate strict regression lines
ggplot(Data, aes(x = Emotional_Score, y = BMIp, color = Picky_Category)) +
  geom_point() + 
  geom_smooth(aes(group = Picky_Category, color = Picky_Category), 
              method = "lm", formula = y ~ x, se = FALSE, linetype = "solid") +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, linetype = "solid", color = "black") + # Overall regression line
  ...
  scale_color_manual(values = c("Low" = "blue", "Medium" = "green", "High" = "red"),
                     name = "Picky Eating Category") +
  annotate("text", x = max(Data$Emotional_Score), y = min(Data$BMIp), 
           label = "Overall Effect of Picky Eating", hjust = 1, vjust = 0) +
  theme_minimal()
...
# Calculate the correlation matrix of predictors
 correlation_matrix <- cor(Data[c("Picky_Score", "Emotional_Score", "SNAP", "Sex", "Income", "Education", "Relation", "household", "Relationship", "num_children")])
