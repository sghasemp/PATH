...
ggplot(ECfinal, aes(x = Control_Score_log, y = Picky_Score_log, color = factor(SNAP))) +
  ...
  labs(title = "Control Score vs. Picky Eating by SNAP",
       ...
       color = "SNAP Status") +
  theme_minimal() +
  scale_color_manual(values = c("blue", "red"))  # Specify colors for SNAP 0 and SNAP 1
...
# Categorize 'Picky_Score_log' into three groups (Low, Medium, High)
Data <- Data %>%
  mutate(Picky_Category = cut(Picky_Score_log,
                              breaks = quantile(Picky_Score_log, probs = c(0, 1/3, 2/3, 1), na.rm = TRUE),
                              labels = c("Low", "Medium", "High"),
                              include.lowest = TRUE))

# Plot with separate regression lines for each level of picky eating and one for the overall data
ggplot(Data, aes(x = Control_Score_log, y = BMIpercentile)) +
  geom_point(aes(color = Picky_Category), alpha = 0.4) +  # Color code points based on picky eating category
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, linetype = "dotted", color = "black") +  # Overall regression line
  geom_smooth(data = Data %>% filter(Picky_Category == "Low"), method = "lm", formula = y ~ x, se = FALSE, color = "blue") +
  geom_smooth(data = Data %>% filter(Picky_Category == "Medium"), method = "lm", formula = y ~ x, se = FALSE, color = "green") +
  geom_smooth(data = Data %>% filter(Picky_Category == "High"), method = "lm", formula = y ~ x, se = FALSE, color = "red") +
  labs(title = "Control Score vs. BMI Percentile Across Picky Eating Categories",
