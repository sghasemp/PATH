...
outcome_model <- lm(Picky_Score ~ Control_Score + SNAP +
                      Sex + Income + Education + Gender + household + Relationship,
                    data = ECfinal)

# Step 3: Perform Mediation Analysis
med_fit <- mediate(mediator_model, outcome_model,
                   treat = "Control_Score", mediator = "SNAP",
                   boot = TRUE)
...
numeric_predictors <- c("Control_Score", "Sex", "Relationship", "Income", "Education", "Gender", "household")


# If any numeric predictors are factors, convert them to numeric
ECfinal[numeric_predictors] <- lapply(ECfinal[numeric_predictors], function(x) {
  if (is.factor(x)) {
...
outcome_model2 <- lm(Picky_Score ~ Emotional_Score + SNAP + 
              Sex + Income + Education + Gender + household+ Relationship ,data = ECfinal)


# Perform the mediation analysis
med_fit2 <- mediate(mediator_model2, outcome_model2, treat = c("Emotional_Score"), mediator = "SNAP", boot = TRUE)
...
numeric_predictors2 <- c("Emotional_Score", "Sex", "Relationship", "Income", "Education", "Gender", "household")


# If any numeric predictors are factors, convert them to numeric
ECfinal[numeric_predictors2] <- lapply(ECfinal[numeric_predictors2], function(x) {
  if (is.factor(x)) {
    as.numeric(as.character(x))
  } else {
    x
