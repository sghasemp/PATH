...
mediator_model <- glm(SNAP ~ Control_Score + Sex + Relationship+
                        Income + Education + Gender + household,
                      family = binomial, data = ECfinal)
#Check Multicollinearity model

numeric_cols <- c("Control_Score", "Relationship","Sex", "Income", "Education", "Gender", "household")
...
#model2
mediator_model2 <- glm(SNAP ~ Emotional_Score + Sex + Relationship+
                        Income + Education + Gender + household,
                      family = binomial, data = ECfinal)
#Check Multicollinearity model2

numeric_cols2 <- c("Emotional_Score", "Relationship","Sex", "Income", "Education", "Gender", "household")

# Convert columns to numeric 
ECfinal[numeric_cols2] <- lapply(ECfinal[numeric_cols2], function(x) as.numeric(as.character(x)))
#create correlation matrixs to assess multi clearity
cor_matrix2 <- cor(ECfinal[numeric_cols2], use = "pairwise.complete.obs")
print(cor_matrix2)




# Adjusting the outcome model(control)  to include covariates
outcome_model <- lm(Picky_Score ~ Control_Score + SNAP + 
                      Control_Score + Sex + Income + Education + Gender + household+ Relationship ,data = ECfinal)
#chek multiclirity
# Correlation Matrix
cor_matrix <- cor(ECfinal[c("Control_Score", "SNAP", "Sex", "Income", "Education", "Gender", "Relationship","household")], use = "complete.obs")
print(cor_matrix)



# Adjusting the outcome model2 (Emotional)to include covariates
outcome_model2 <- lm(Picky_Score ~ Emotional_Score + SNAP + 
                      Control_Score + Sex + Income + Education + Gender + household+ Relationship ,data = ECfinal)
#chek multiclirity
# Correlation Matrix
cor_matrix2 <- cor(ECfinal[c("Emotional_Score", "SNAP", "Sex", "Income", "Education", "Gender", "Relationship","household")], use = "complete.obs")
print(cor_matrix2)
...
# Perform the mediation analysis
med_fit2 <- mediate(mediator_model2, outcome_model2, treat = c("Emotional_Score"), mediator = "SNAP", boot = TRUE, sims = 500)
...
glm<- glm(BMIpercentile ~ Picky_Score + Control_Score + 
                           Picky_Score * Control_Score+ SNAP + 
                      Control_Score + Sex + Income + Education + Gender + household+ Relationship, family = quasibinomial(link = "logit"), data = Data)



#  Check for Multicollinearity
variables_for_cor <- c("Picky_Score", "SNAP", "Control_Score", "Sex", "Income", "Education", "Gender", "household","Relationship")
#ensure numeric type
...
glm2 <- glm(BMIpercentile ~ Picky_Score + Emotional_Score + 
                           Picky_Score * Emotional_Score+ SNAP + Sex + Income + Education + Gender + household+ Relationship, family = quasibinomial(link = "logit"), data = Data)



#  Check for Multicollinearity
variables_for_cor2 <- c("Picky_Score", "Emotional_Score", "SNAP", "Sex", "Income", "Education", "Gender", "household","Relationship")
#ensure numeric type
Data[variables_for_cor2] <- lapply(Data[variables_for_cor2], function(x) as.numeric(as.character(x)))
# imputation missing data
Data_imputed2 <- Data
for (col in variables_for_cor2) {
  Data_imputed[[col]] <- ifelse(is.na(Data_imputed2[[col]]), mean(Data_imputed2[[col]], na.rm = TRUE), Data_imputed2[[col]])
}

# Calculate correlation matrix
# Using the dataset with handled missing values (choose Data_complete or Data_imputed)
cor_matrix2 <- cor(Data_imputed2[variables_for_cor2], use = "pairwise.complete.obs")
print(cor_matrix2)
