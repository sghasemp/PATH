...
library(dplyr)
#read date
#Updoaded data from qualtrics
dataset <- read.csv("C:/Users/sara ghasem pour/GitLab/Path2/Thesis/RawData/EC_pren.csv")
dataset2<- read.csv("C:/Users/sara ghasem pour/GitLab/Path2/Thesis/RawData/ECn.csv")
...
dataset$StartDate <- as.POSIXct(dataset$StartDate, format = "%Y-%m-%d")
dataset$EndDate <- as.POSIXct(dataset$EndDate, format = "%Y-%m-%d")
dataset2$StartDate <- as.POSIXct(dataset2$StartDate, format = "%Y-%m-%d")
dataset2$EndDate <- as.POSIXct(dataset2$EndDate, format = "%Y-%m-%d")

# Define cutoff date
cutoff_date <- as.POSIXct("2023-09-19")

# Filter out data for participants after the cutoff date
filtered_data <- subset(dataset2, EndDate <= cutoff_date)

# Combine dataset and filtered_data into ECcom
...
# Convert "m/d/y" to "y-m-d" format in ECfinal$child_bday column
ECfinal$child_bday <- as.Date(ECfinal$child_bday, format = "%m/%d/%y")
ECfinal$child_bday <- format(ECfinal$child_bday, "%Y-%m-%d")


ECfinal$RecordedDate <- as.Date(ECfinal$RecordedDate, format="%Y-%m-%d")
...
# Remove the third row from ECfinal
ECfinal <- ECfinal[-2, ]
